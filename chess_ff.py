...
def pawn(piece, board):
    ...
    if piece.color == "Black":
        x = piece.xpos
        y = piece.ypos
        if piece.first == True:#if it is thez
            ...
            if board[i][j] != None:
                if board[i][j].color == color:
                    if board[i][j].typ == "Queen":
                        ...
                        if opt==1:#to add king moves
                            possible.extend( getKing(board[i][j], board) )
    return possible    
def check(king, board):
    if king.color == "White":
            moves = all("Black", board,1)
    else:
            moves = all("White", board,1)
    attacking = 0
    for move in moves:
...
def all2(color, board,opt):
    possible = all(color, board,opt)
    # find king
    king = None
    for i in range(0, 8):
        for j in range(0, 8):
